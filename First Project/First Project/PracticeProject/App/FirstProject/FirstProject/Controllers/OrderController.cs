...
namespace FirstProject.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        {
            _orderService = orderService;  //product ma hm ny quantity increase krdi hy ai samjh//two  time create horaha hy ai samjh hmmm  a gyii //lakin ordernumber to same nhi ana chaye hm ny us mn increment lgya tha
        }
        
            [HttpPost("GetAllOrderList")]
        public async Task<List<Order>> CreateList(string product, string customer, decimal quantity)    
        {
            var result = await _orderService
                .GetAllOrderList(product, customer, quantity);

            return result.Data;
        }
        [HttpPost("createorderlist")]
        public async Task<IActionResult> Create(string product,string customer, decimal quantity)
        {
            var result = await _orderService
                .CreateOrderAsync(product, customer, quantity);
        ...
        //public async Task <ResponseModel> getallcustomerorderproduct(int id) { 
